...
def generate_us_spark():
    ...
    uscolumns=["Number of Veterans","Race","Count"]
    # Drop columns we don't need and drop duplicate rows
    us=us_spark.drop(*uscolumns).dropDuplicates()

    # Saving (committing) joined US dataset
...
def create_i94non_immigrant_port_entry_parquet(us, i94port):
    ...
    # Drop duplicate rows for i94_spark
    ...
    # NOTE: We use left join against city records which may cause null values because
    # we may not have demographic statistics on all U.S. ports of entry
...
def create_i94date_parquet(i94date):
    # Add Sessons to i94date dimension:
...
us = generate_us_spark()
create_i94visa_parquet()
create_i94res_parquet()
create_i94mode_parquet()
i94port = create_i94port_parquet()
i94date = create_i94non_immigrant_port_entry_parquet(us, i94port)
create_i94date_parquet(i94date)
